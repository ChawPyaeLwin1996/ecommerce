<template>
	...
	<div class="bg-white border border-transparent hover:border-primary-500 rounded-primary overflow-hidden mx-auto relative product p-2">
		...
		<div class="bg-white flex justify-center item-center product-image relative rounded-primary">
			<img :src="product.image" class="h-full w-full" alt="product" style="object-fit: cover;">

			<!-- options on hover -->
			<div class="product-options absolute flex items-center justify-center">

				<!-- add to cart button -->
				<button @click="goToPlace()"></button>
			...
			<div class="absolute top-3 right-3">
				...
				<span class="bg-red-500 text-white text-xs px-3 py-1 rounded-full uppercase tracking-wider">{{product.name}}</span>
			</div>
		</div>

		<!-- details -->
		<div class="flex items-center justify-between p-4">
			<p class="text-grey-300 whitespace-nowrap text-sm">Price : {{product.lessAmount}} ~ {{product.biggestAmount}}</p>
		</div>
